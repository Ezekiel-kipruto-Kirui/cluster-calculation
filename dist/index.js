...
const ensureBackendEnvLoaded = () => {
    ...
    if (typeof loadEnvFile !== "function")
        return;
    const hasMpesaCallbackConfig = Boolean(process.env.MPESA_CALLBACK_URL || process.env.MPESA_CALLBACK_URL_LOCAL || process.env.MPESA_CALLBACK_URL_FIREBASE);
    if (hasMpesaCallbackConfig)
        return;
    const envCandidates = [
        path_1.default.resolve(process.cwd(), ".env"),
        path_1.default.resolve(process.cwd(), "backend-server", ".env"),
...
const logger = {
    info: (message, payload) => console.log(message, payload || ""),
...
const ensureRequestId = (request) => {
    ...
    const headerRequestId = request.headers?.["x-request-id"] || request.headers?.["x-correlation-id"] || "";
    const requestId = headerRequestId ? String(headerRequestId) : crypto_1.default.randomUUID();
    request.requestId = requestId;
    return requestId;
};
const adminSessionsById = new Map();
...
const setAdminSessionCookie = (response, sessionId) => {
...
const createAdminSession = ({ uid, email, profile, }) => {
...
const requireAdminSession = (request, response, next) => {
...
const universityKey = (entry) => `${String(entry?.name || "").trim().toLowerCase()}|${String(entry?.courseCode || "").trim().toLowerCase()}`;
...
const upsertSingleCourseCatalogEntryInDb = async ({ cluster, name, requirements, universities, }) => {
    ...
    const existingIndex = clusterCourses.findIndex((course) => String(course?.name || "").trim().toLowerCase() === normalizedName.toLowerCase());
    const mergedCourse = existingIndex >= 0
        ? {
            name: normalizedName,
            requirements: {
    ...
    if (existingIndex >= 0)
        clusterCourses[existingIndex] = mergedCourse;
    else
        clusterCourses.push(mergedCourse);
...
const fetchAllClusterSessionsFromDb = async () => {
...
const updateClusterSessionByCodeInDb = async (code, patch = {}) => {
    const normalizedCode = normalizeSessionCode(code);
    if (!normalizedCode)
    ...
    const payload = {};
    if ("email" in patch)
        payload.email = String(patch.email || "").trim();
    if ("phoneNumber" in patch)
        payload.phoneNumber = String(patch.phoneNumber || "").trim();
    if ("medicineEligible" in patch)
        payload.medicineEligible = Boolean(patch.medicineEligible);
    if ("grades" in patch)
        payload.grades = normalizeSessionGrades(patch.grades);
    if ("results" in patch)
        payload.results = normalizeSessionResults(patch.results);
    if ("paymentResponse" in patch)
        payload.paymentResponse = patch.paymentResponse ?? null;
...
const deleteClusterSessionByCodeInDb = async (code) => {
    const normalizedCode = normalizeSessionCode(code);
    if (!normalizedCode)
        throw new Error("Session code is required.");
    await getRealtimeDb().ref(`${sessionsPath}/${normalizedCode}`).remove();
};
const deleteClusterSessionsByCodesInDb = async (codes = []) => {
    const normalizedCodes = Array.from(new Set((Array.isArray(codes) ? codes : []).map((code) => normalizeSessionCode(code)).filter(Boolean)));
...
const fetchAdminProfileByUid = async (uid) => {
...
const upsertAdminProfileByUid = async (uid, patch) => {
    const normalizedUid = String(uid || "").trim();
    if (!normalizedUid)
...
const ensureAdminProfileForUser = async ({ uid, email, displayName, }) => {
...
const parseFirebaseAuthError = (payload, fallback) => {
...
const signInWithEmailPasswordViaFirebase = async ({ email, password, }) => {
    ...
    if (!idToken)
        throw new Error("Login failed: missing idToken.");
    const uid = String(payload?.localId || "").trim();
    if (!uid)
        throw new Error("Login failed: missing Firebase uid.");
...
const createEmailPasswordUserViaFirebase = async ({ email, password, displayName, }) => {
    const apiKey = getFirebaseApiKey();
    ...
    if (!uid)
        throw new Error("Firebase user creation failed: missing uid.");
...
const paymentSessionsByCheckoutId = new Map();
const checkoutIdByMerchantRequestId = new Map();
const paymentSessionTtlMs = Number(getEnv("PAYMENT_SESSION_TTL_MS", `${6 * 60 * 60 * 1000}`)) || 6 * 60 * 60 * 1000;
const normalizeRequestIdentifier = (value) => String(value || "").trim();
...
const savePaymentSession = (session) => {
...
const upsertPaymentSession = (patch) => {
    cleanupExpiredPaymentSessions();
    ...
    const resolvedCheckoutRequestId = normalizedCheckoutRequestId || checkoutIdByMerchantRequestId.get(normalizedMerchantRequestId) || "";
    if (!resolvedCheckoutRequestId)
        return null;
    ...
    const session = {
        ...
        status: (String(patch?.status || existing?.status || "pending").trim() || "pending"),
        resultCode: patch?.resultCode !== undefined && patch?.resultCode !== null
...
const metadataArrayToObject = (items) => Array.isArray(items)
    ...
    : {};
const parseDarajaCallback = (body) => {
    ...
    return {
        ...
        resultCode: callback?.ResultCode !== undefined && callback?.ResultCode !== null ? Number(callback.ResultCode) : null,
        resultDesc: String(callback?.ResultDesc || "").trim(),
        metadata,
    };
};
const mapDarajaResultCodeToStatus = (resultCode) => {
...
const normalizeGradesPayload = (value) => {
    if (!value || typeof value !== "object" || Array.isArray(value)) {
        throw new Error("grades must be an object keyed by subject code.");
    }
    return Object.entries(value).reduce((accumulator, [subjectCode, grade]) => {
...
const clusterCacheMaxEntries = Math.max(0, parseNumberEnv(getEnv("CLUSTER_CACHE_MAX", "200"), 200));
const clusterCacheTtlMs = Math.max(0, parseNumberEnv(getEnv("CLUSTER_CACHE_TTL_MS", `${10 * 60 * 1000}`), 10 * 60 * 1000));
...
const clusterResultsCache = new Map();
...
const getClusterCache = (cacheKey) => {
    if (!clusterCacheEnabled)
        return null;
...
const calculateClusterResults = async (grades) => {
    ...
    if (clusterCacheEnabled) {
        ...
        const value = {
            ...
            results: (0, clusterEngine_1.computeAllClusters)(normalizedGrades),
            medicineEligible: (0, clusterEngine_1.medicineEligibility)(normalizedGrades),
        };
...
const hasValidFirebasePublicConfig = (config) => Boolean(config.apiKey && config.authDomain && config.databaseURL && config.projectId);
const getDarajaToken = async () => {
    ...
    try {
        ...
        const wrapped = new Error(`Unable to reach Daraja auth endpoint. ${error?.message || "Network request failed."}`);
...
const getDarajaTransactionType = () => {
    ...
    if (shortcodeType === "tillnumber" ||
        shortcodeType === "buygoods" ||
        shortcodeType === "customerbuygoodsonline") {
        return "CustomerBuyGoodsOnline";
    }
    const error = new Error("Invalid MPESA_SHORTCODE_TYPE. Use paybill, till_number, CustomerPayBillOnline, or CustomerBuyGoodsOnline.");
    error.statusCode = 500;
    throw error;
};
const requestStkPush = async ({ phone, amount, accountReference, transactionDesc, }) => {
    ...
    if (!callbackUrl) {
        const error = new Error("M-Pesa callback URL is not configured. Set MPESA_CALLBACK_URL, MPESA_CALLBACK_URL_LOCAL, or MPESA_CALLBACK_URL_FIREBASE on backend.");
        error.statusCode = 500;
        throw error;
    }
    const shortCode = requireEnv("MPESA_SHORTCODE");
    const businessShortCode = firstEnv("MPESA_BUSINESS_SHORTCODE", "MPESA_EXPRESS_SHORTCODE", "MPESA_BUSINESS_SHORT_CODE", "MPESA_SHORTCODE") ||
        shortCode;
    ...
    try {
        ...
        const wrapped = new Error(`Unable to reach Daraja STK endpoint. ${error?.message || "Network request failed."}`);
        wrapped.statusCode = 502;
        throw wrapped;
    }
    const payload = await response.json().catch(() => ({}));
...
const queryStkPushStatus = async ({ checkoutRequestId }) => {
    ...
    try {
        ...
        const wrapped = new Error(`Unable to reach Daraja query endpoint. ${error?.message || "Network request failed."}`);
        wrapped.statusCode = 502;
        throw wrapped;
    }
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) {
        const error = new Error(payload?.errorMessage || payload?.errorCode || `Daraja STK query failed with HTTP ${response.status}.`);
        error.statusCode = response.status;
        throw error;
    }
    return payload;
...
const firebaseConfigHandler = async (request, response) => {
...
const publicConfigHandler = async (_request, response) => {
    response.status(200).json({
        payableAmount,
...
const adminMeHandler = async (request, response) => {
...
const adminLogoutHandler = async (request, response) => {
    const sessionId = getSessionIdFromRequest(request);
    if (sessionId)
        adminSessionsById.delete(sessionId);
...
const adminHealthHandler = async (request, response) => {
    const adminRequest = request;
    const session = adminRequest.adminSession || null;
    if (!session) {
        response.status(401).json({ error: "Admin authentication is required." });
...
const addRegularAdminHandler = async (request, response) => {
    const adminRequest = request;
    const session = adminRequest.adminSession || null;
    if (!session) {
        response.status(401).json({ error: "Admin authentication is required." });
...
const fetchCourseCatalogHandler = async (_request, response) => {
    const catalog = await fetchCourseCatalogFromDb();
    response.status(200).json(catalog);
};
const uploadCourseCatalogHandler = async (request, response) => {
    if (!assertMethod(request, response, "POST"))
        return;
    await uploadCourseCatalogToDb(getBody(request));
    response.status(200).json({ success: true });
};
const upsertSingleCourseHandler = async (request, response) => {
    if (!assertMethod(request, response, "POST"))
        return;
    const body = getBody(request);
...
const saveClusterSessionHandler = async (request, response) => {
    if (!assertMethod(request, response, "POST"))
        return;
    const body = getBody(request);
...
const fetchClusterSessionByCodeHandler = async (request, response) => {
...
const fetchAllClusterSessionsHandler = async (_request, response) => {
    const sessions = await fetchAllClusterSessionsFromDb();
    response.status(200).json(sessions);
};
const updateClusterSessionHandler = async (request, response) => {
    const code = String(request.params?.code || "").trim();
    await updateClusterSessionByCodeInDb(code, getBody(request));
    response.status(200).json({ success: true });
};
const deleteClusterSessionHandler = async (request, response) => {
    const code = String(request.params?.code || "").trim();
    await deleteClusterSessionByCodeInDb(code);
    response.status(200).json({ success: true });
};
const deleteClusterSessionsBulkHandler = async (request, response) => {
    if (!assertMethod(request, response, "POST"))
        return;
    const body = getBody(request);
...
const apiPaymentsCreateHandler = async (request, response) => {
    logHandlerAccess("apiPaymentsCreate", request);
    if (!assertMethod(request, response, "POST"))
        return;
    const body = getBody(request);
    const phone = body?.phone_number || body?.phoneNumber || body?.phone || body?.["phone number"] || body?.phone_number || "";
...
const apiPaymentsQueryHandler = async (request, response) => {
    ...
    try {
        ...
        const transientDarajaError = statusCode >= 500 || /temporarily unavailable|timeout|timed out|service unavailable|internal/i.test(message);
        if (existingSession) {
            response.status(200).json({
                success: true,
                data: {
        ...
        if (!missingTransaction && !transientDarajaError)
            throw error;
        const paymentSession = upsertPaymentSession({
            checkoutRequestId,
...
const apiPaymentsCallbackHandler = async (request, response) => {
...
const apiPaymentsValidationHandler = async (_request, response) => {
    response.status(200).json({
        ResultCode: 0,
...
const stkPushHandler = async (request, response) => {
    ...
    try {
        const body = getBody(request);
        const phone = body["phone number"] || body.phone_number || body.phoneNumber || body.phone || "";
...
const paymentStatusHandler = async (request, response) => {
    ...
    const input = request.method === "GET" ? request.query || {} : getBody(request);
...
const resolvePaymentSessionForCalculation = async ({ checkoutRequestId, merchantRequestId, }) => {
    const normalizedCheckoutRequestId = normalizeRequestIdentifier(checkoutRequestId);
    const normalizedMerchantRequestId = normalizeRequestIdentifier(merchantRequestId);
    ...
    const resolvedCheckoutRequestId = normalizedCheckoutRequestId ||
    ...
    try {
        ...
        return upsertPaymentSession({
            checkoutRequestId: resolvedCheckoutRequestId,
            merchantRequestId: normalizedMerchantRequestId || existingSession?.merchantRequestId || queryResponse?.MerchantRequestID || "",
            phone: existingSession?.phone || "",
            amount: existingSession?.amount || 0,
            status,
            resultCode: Number.isFinite(resultCode) ? resultCode : null,
...
const calculateClusterPointsHandler = async (request, response) => {
...
const createBackendServer = () => {
    ...
    app.post("/api/admin/catalog/upload", requireAdminSession, withAsyncGuard("uploadCourseCatalog", uploadCourseCatalogHandler));
    app.post("/api/admin/catalog/course", requireAdminSession, withAsyncGuard("upsertSingleCourse", upsertSingleCourseHandler));
    ...
    app.patch("/api/admin/sessions/:code", requireAdminSession, withAsyncGuard("updateClusterSession", updateClusterSessionHandler));
    app.delete("/api/admin/sessions/:code", requireAdminSession, withAsyncGuard("deleteClusterSession", deleteClusterSessionHandler));
    app.post("/api/admin/sessions/delete-many", requireAdminSession, withAsyncGuard("deleteClusterSessionsBulk", deleteClusterSessionsBulkHandler));
    ...
    app.use((error, request, response, next) => {
        ...
        const statusCode = Number(error?.statusCode || error?.status || 500) >= 400
